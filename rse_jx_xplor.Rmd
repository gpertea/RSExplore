...
if (nrow(xovls)!=length(xgrsub)) {
  ...
  #  summarize( tgene=paste0(ggr[subjectHits]$ogname, collapse = '|'))
...
table(rse_exon$Dx)
nrow(rse_exon)
# save this file to be merged later

```

Select transcripts 
```{r}
rse_tx_file <- "data/astellas_dg_hg38_rseTx_n263.rda"
load(rse_tx_file)
###TODO check - do we ever need to process anti-sense exons? (ignore.strand = T)?
...
rse_tx <- rse_tx[, rse_tx$Dx %in% c('Schizo', 'Control')]
table(rse_tx$Dx)
nrow(rse_tx)
